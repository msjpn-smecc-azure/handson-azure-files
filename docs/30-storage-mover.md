...
Azure Storage Mover を使用して
オンプレミスのファイルサーバーから Azure Files へデータを移行する方法を学習します。
Log Analytics ワークスペースの作成、Storage Mover リソースの構成、エージェントのデプロイと登録、
エンドポイント設定、および移行プロジェクトの実施を通じて、エンタープライズレベルのファイル移行方法を習得します。


#### 🗒️ 目次

1. [移行作業のログ保管ストレージ作成](#移行作業のログ保管ストレージ作成)
1. [Azure Storage Mover リソースの作成](#azure-storage-mover-リソースの作成)
1. [Storage Mover エージェント をデプロイ](#storage-mover-エージェント-をデプロイ)
1. [Storage Mover エージェントを登録](#storage-mover-エージェントを登録)
1. [キーコンテナーを作成](#キーコンテナーを作成)
1. [ソースエンドポイントを作成](#ソースエンドポイントを作成)
1. [ターゲットエンドポイントを作成](#ターゲットエンドポイントを作成)
1. [移行プロジェクトを構成](#移行プロジェクトを構成)
1. [ファイル移行の確認](#ファイル移行の確認)
...
1. ソースエンドポイントを作成
    ...
    - ソースの種類: `SMB`
    - ホスト名またはIP: `10.1.0.5` (ファイル共有サーバー `onpre-file-srv` のプライベートIPアドレス)
...
> [!IMPORTANT]  
> `Permission denied` エラーが発生した場合、ユーザー名またはパスワードに誤りがある可能性があります。
> Key Vault に登録した **シークレット値の再登録** を試します。
> なお、再登録した際は **URL を指定してジョブを更新** します。
